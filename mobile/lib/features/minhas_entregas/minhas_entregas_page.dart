...
class _MinhasEntregasPageState extends State<MinhasEntregasPage> {
  ...
  Future<void> _carregar() async {
    ...
    final offlineService = context.read<OfflineOperationService>();
    try {
      try {
        await proofService.sincronizar();
      } catch (_) {}
      final result = await offlineService.carregar();
      final proofs = await proofService.quantidadePendente();
      if (!mounted) return;
      setState(() {
        _entregas = result.entregas;
        _offline = result.offline;
        _pendentes = result.acoesPendentes + proofs;
        _carregando = false;
      });
  ...
  Future<void> _avancar(Entrega entrega, StatusEntrega proximo) async {
    ...
    try {
      final synced = await context
          .read<OfflineOperationService>()
          .alterarStatus(entrega.id, proximo);
      if (!mounted) return;
      mostrarMensagem(
          context,
          synced
      ...
      if (!mounted) return;
      setState(
          () => _erro = context.read<ApiClient>().translate(error).message);
    } finally {
  ...
  Future<void> _external(Future<bool> Function() action) async {
    ...
    if (!opened && mounted) {
      mostrarMensagem(context, 'Nenhum aplicativo compatível foi encontrado.');
    }
  }
  ...
  Widget build(BuildContext context) => Scaffold(
        ...
        body: RefreshIndicator(
            color: AppColors.amber, onRefresh: _carregar, child: _conteudo()),
      );
  ...
  Widget _conteudo() {
    ...
    if (_entregas.isEmpty) {
      return ListView(
          children: const [EmptyState('Nenhuma entrega designada para você.')]);
    }
    return ListView.separated(
      ...
      itemBuilder: (context, index) {
        ...
        return Semantics(
          container: true,
          label: 'Entrega ${entrega.codigo}, status ${entrega.status.label}',
          child: PanelCard(
              child: Column(
                ...
                Wrap(spacing: 8, runSpacing: 8, children: [
                  OutlinedButton.icon(
                      onPressed: () =>
                          _external(() => actions.abrirMapa(origem)),
                      icon: const Icon(Icons.trip_origin),
                      label: const Text('Mapa origem')),
                  OutlinedButton.icon(
                      onPressed: () =>
                          _external(() => actions.abrirMapa(destino)),
                      icon: const Icon(Icons.place),
                      label: const Text('Mapa destino')),
                  OutlinedButton.icon(
                      onPressed: () async {
                        await actions.copiarEndereco(destino);
                        if (!context.mounted) {
                          return;
                        }
                        mostrarMensagem(context, 'Endereço copiado.');
                      },
                  ...
                  if (entrega.destinatarioTelefone.isNotEmpty) ...[
                    OutlinedButton.icon(
                        onPressed: () => _external(
                            () => actions.ligar(entrega.destinatarioTelefone)),
                        icon: const Icon(Icons.phone),
                        label: const Text('Ligar')),
                    OutlinedButton.icon(
                        onPressed: () => _external(() =>
                            actions.whatsapp(entrega.destinatarioTelefone)),
                ...
                if (proximo != null) ...[
                  ...
                  SizedBox(
                      width: double.infinity,
                      child: FilledButton(
                        onPressed: _atualizandoId == null
                            ? () => _confirmarAvanco(entrega)
                            : null,
                        child: Text(_atualizandoId == entrega.id
                            ? 'Atualizando...'
                            : _acao(proximo)),
                      )),
                ],
