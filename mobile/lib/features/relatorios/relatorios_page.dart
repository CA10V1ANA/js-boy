...
class _RelatoriosPageState extends State<RelatoriosPage> {
  ...
  Widget build(BuildContext context) {
    final entregues = _entregas
        .where((entrega) => entrega.status == StatusEntrega.entregue)
        .length;
    final canceladas = _entregas
        .where((entrega) => entrega.status == StatusEntrega.cancelada)
        .length;
    final ticketMedio = _relatorio.pagamentosRegistrados > 0
        ? _relatorio.valorRecebido / _relatorio.pagamentosRegistrados
        : 0.0;
    final taxaConclusao =
        _entregas.isNotEmpty ? (entregues / _entregas.length * 100).round() : 0;

    final contagemFormas = <FormaPagamento, int>{};
    for (final pagamento in _pagamentos) {
      contagemFormas[pagamento.formaPagamento] =
          (contagemFormas[pagamento.formaPagamento] ?? 0) + 1;
    }
    ...
    for (final entrega in _entregas) {
      contagemClientes[entrega.clienteNome] =
          (contagemClientes[entrega.clienteNome] ?? 0) + 1;
    }
                ...
                children: [
                  GridView.count(
                    ...
                    children: [
                      ...
                      _metrica(
                          'Concluidas', '$entregues de ${_entregas.length}'),
                                    ...
                                    children: [
                                      Row(
                                        ...
                                        children: [
                                          ...
                                          Text(
                                              '${(item.value / _pagamentos.length * 100).round()}%',
                                              style: GoogleFonts.hankenGrotesk(
                                                  fontSize: 12.5,
                                                  fontWeight: FontWeight.w600,
                                      ...
                                      ClipRRect(
                                        ...
                                        child: LinearProgressIndicator(
                                          value:
                                              item.value / _pagamentos.length,
                                          minHeight: 8,
                                          backgroundColor: AppColors.rowDivider,
                                          valueColor: AlwaysStoppedAnimation(
                                              _coresForma[item.key] ??
                                                  AppColors.muted),
                                        ),
                                      ),
                                    ],
      ...
      child: Column(
        ...
        children: [
          ...
          if (nota != null) ...[
            ...
            Text(nota,
                style: GoogleFonts.hankenGrotesk(
                    fontSize: 11, color: const Color(0xFFC67A15))),
          ],
        ],
      ),
    );
